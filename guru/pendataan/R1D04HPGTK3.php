...
// -------------------------------------------------- Class --------------------------------------------------
class R1D04HPGTK3class
{
	function R1D04HPGTK3()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_hdrkmnps_pgtk3.*
						FROM 		t_hdrkmnps_pgtk3
						WHERE 		t_hdrkmnps_pgtk3.nis		='". mysql_escape_string($nis)."' AND 
									t_hdrkmnps_pgtk3.thn_ajaran		='". mysql_escape_string($thnajr)."'	"; // menghasilkan nilai kehadiran per siswa
		...
		<FORM ACTION='guru.php' METHOD='get' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD COLSPAN='2'><B>INPUT COMMENT (3. Mathematical/Cognitive Development)</B></TD></TR>
				<TR></TR><TR></TR>
				...
				<tr>
					<td>Tahun Pelajaran</td><td> : 
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04HPGTK3'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			...
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit Comment' 	onClick=window.location.href='guru.php?mode=R1D04HPGTK3&kdekls=$kdekls&thnajr=$thnajr&pilihan=komentar'>";
			...
			if($pilihan=='komentar')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04HPGTK3_Save_Komentar'>
	...
	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HPGTK3_Save()
	
	// -------------------------------------------------- Save --------------------------------------------------
	function R1D04HPGTK3_Save_Komentar()
	{
		$kdekls	=$_POST['kdekls'];
		$sms	=$_POST['sms'];
		$nlitrm	=$_POST['nlitrm'];
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_hdrkmnps_pgtk3.nis		='". mysql_escape_string($nis)."',
								t_hdrkmnps_pgtk3.kmn"."$sms"."$nlitrm"."	='". mysql_escape_string($kmn)."',
								t_hdrkmnps_pgtk3.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_hdrkmnps_pgtk3.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_hdrkmnps_pgtk3.jamrbh	='". mysql_escape_string($jamrbh)."', 
								t_hdrkmnps_pgtk3.thn_ajaran	='". mysql_escape_string($thnajr)."' "; 

			$query	="	SELECT 		t_hdrkmnps_pgtk3.*
						FROM 		t_hdrkmnps_pgtk3
						WHERE 		t_hdrkmnps_pgtk3.nis	='". mysql_escape_string($nis)."'  AND 
									t_hdrkmnps_pgtk3.thn_ajaran	='". mysql_escape_string($thnajr)."'  ";
			$result =mysql_query($query);
			...
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_hdrkmnps_pgtk3 ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan"));
			}
			else
			{
				$query 	="	UPDATE 	t_hdrkmnps_pgtk3 ".$set.
						"	WHERE 	t_hdrkmnps_pgtk3.nis	='". mysql_escape_string($nis)	."'  AND 
									t_hdrkmnps_pgtk3.thn_ajaran	='". mysql_escape_string($thnajr)."' ";
		...
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04HPGTK3&kdekls=$kdekls&thnajr=$thnajr&nis=$nis&pilihan=detil\">\n";
 	}		

	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HPGTK3_Save_Kenaikan()
