...
while($y<$x)
{
	...
	$pdf->Cell(17.5	,0.6, "SENIOR HIGH SCHOOL",0,0,C); // 19 $judul
	$pdf->SetFont('Arial','B',16);
	$pdf->Ln(0.65);
	
	$pdf->Cell(17.5	,0.6, "STUDENT'S PROGRESS REPORT",0,0,C); // 19 $judul2 .substr($kdekls,-2)
	$pdf->Ln();
	
	$pdf->SetFont('Arial','B',14);
	$pdf->Cell(17.5	,0.6, "ACADEMIC YEAR ".$thnajr,0,0,C);// 19 ." - SAINT JOHN'S SCHOOL"
	$pdf->Ln();
	...
	$pdf->Cell( 2.5	,0.6,"Semester",0,0,L); 
	$pdf->Cell( 0.5	,0.6," :",0,0,C); 
	$pdf->Cell( 5.7	,0.6,$sms,0,0,L); //1.5
	$pdf->Cell( 0.5	,0.6,"",0,0,L); 
	$pdf->Cell( 1.5	,0.6,"Term",0,0,L); 
	$pdf->Cell( 0.5	,0.6," :",0,0,C); 
	$pdf->Cell( 1.5	,0.6,$midtrm,0,0,L); //".$sms."=2
	
	$pdf->Ln(0.5);
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 1.5	,0.6,"",0,0,L); 
	$pdf->Cell( 2.5	,0.6,"Student Name",0,0,L); 
	$pdf->Cell( 0.5	,0.6,"  :",0,0,C); 
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 5.7	,0.6,$nmassw,0,0,L); //."                         "
	
	$pdf->Ln(0.5);
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 1.5	,0.6,"",0,0,L); 
	$pdf->Cell( 2.5	,0.6,"Student ID No",0,0,L); 
	$pdf->Cell( 0.5	,0.6," :",0,0,C); 
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 5.7	,0.6,substr($nis,0,3),0,0,L); 
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 0.5	,0.6,"",0,0,L); 
	$pdf->Cell( 1.5	,0.6,"Birthday",0,0,L); 
	$pdf->Cell( 0.5	,0.6," :",0,0,C); 
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 2	,0.6,$tmplhr.", ".$tgllhr,0,0,L); 
	...
	$pdf->Cell( 2	,0.4,$nKKMart		,'LRTB',0,C,true);
	$pdf->Cell( 1	,0.4,$qK		,'LRTB',0,C,true);
	$pdf->SetFillColor(0, 183, 235);
	$pdf->Cell( 1.5	,0.4,$lgK		,'LRTB',0,C,true);
	$pdf->SetFillColor(255, 255, 255);
