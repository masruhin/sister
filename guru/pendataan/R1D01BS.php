...
//Keterangan	: Fungsi-fungsi bsl upload
//----------------------------------------------------------------------------------------------------
if(!defined("sister"))
{
...
class R1D01BSclass
{
	...
	function R1D01BS()
	{
		...
		if ($pilihan=='detil_general'  or $pilihan=='tambah_item' )
		{
			...
			$query 	="	SELECT 	g_gnrbsl.*
						FROM 	g_gnrbsl
						WHERE 	g_gnrbsl.kdebsl='". mysql_escape_string($kdemtrB)."'";
		...
		{	
			$id=$_GET['id'];
			$query1 ="	SELECT 		g_dtlbsl.*
						FROM 		g_dtlbsl
						WHERE 		g_dtlbsl.kdebsl='". mysql_escape_string($kdemtr)."'";
		...
		$query2 ="	SELECT 		g_dtlbsl.*,t_mstkry.*
					FROM  		g_dtlbsl,t_mstkry
					WHERE 		g_dtlbsl.kdebsl='". mysql_escape_string($kdemtr)."'AND
		...
		<BODY onload='sf();'>
			...
			<FORM ID='validasi'  ACTION='guru.php'METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					<TR><TD COLSPAN='2'><B><!--BANK SOAL-->YEAR PLAN</B></TD></TR>
					<TR></TR><TR></TR>
					...
					<INPUT TYPE='hidden' NAME='mode'  id='mode'	VALUE='R1D01BS_Save_Item'></TR>
				</TABLE>	
				...
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					...
					<TR>
						<TD ALIGN='left'>
							<INPUT TYPE='button' VALUE='New' onClick=window.location.href='guru.php?mode=R1D01BS&pilihan=tambah_general'>";

							if ($pilihan=='detil_general')
							{
								echo"
								<INPUT TYPE='button' VALUE='Tambah Materi' onClick=window.location.href='guru.php?mode=R1D01BS&kdemtr=$kdemtr&pilihan=tambah_item'>
								<INPUT TYPE='button' VALUE='Hapus' onClick='confirmation()'>";
							}	
							if ($pilihan=='detil_general' or $pilihan=='tambah_item')
							{
								//echo"<INPUT TYPE='button' VALUE='Print' 	onClick=window.open('pendataan/R1D01BS_Cetak.php?kdemtr=$kdemtr')>";
							}
						echo"
						</TD>
					</TR>
	...
	function R1D01BS_Save()
	{
        ...
        $query1 ="	SELECT 		g_gnrbsl.*
					FROM 		g_gnrbsl
					WHERE
								g_gnrbsl.kdeklm	='". mysql_escape_string($kdeklm)."'	and
								g_gnrbsl.kdeplj	='". mysql_escape_string($kdeplj)."'";

		$result =mysql_query($query1);
		$data1 	=mysql_fetch_array($result);
		if($data1==0)
		{
			$set	="	SET     g_gnrbsl.kdebsl	='". mysql_escape_string($kdemtr)."',
								g_gnrbsl.kdeklm	='". mysql_escape_string($kdeklm)."',
								g_gnrbsl.kdeplj	='". mysql_escape_string($kdeplj)."'";

			$query 	="INSERT INTO g_gnrbsl ".$set;
			$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
        }
 	}
    // -------------------------------------------------- Save Detail --------------------------------------------------
	function R1D01BS_Save_Item()
	{
		require_once '../fungsi_umum/sysconfig.php';
        ...
        $set		="	SET     g_dtlbsl.kdebsl	='". mysql_escape_string($kdemtr)."',
                                g_dtlbsl.kdegru	='". mysql_escape_string($kdegru)."',
		   						g_dtlbsl.nmabsl	='". mysql_escape_string($nmamt)."',
                                g_dtlbsl.stt	='". mysql_escape_string($stt)."',
                                g_dtlbsl.tglinp	='". mysql_escape_string($tglinp)."',
                                g_dtlbsl.jaminp	='". mysql_escape_string($jaminp)."',
								g_dtlbsl.type	='". mysql_escape_string($type[1])."'";

  		$query 	="INSERT INTO g_dtlbsl ".$set;
		$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
		$kdegruB=$kdegru;
		...
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D01BS&kdemtr=$kdemtr&pilihan=tambah_item\">\n";
	}
