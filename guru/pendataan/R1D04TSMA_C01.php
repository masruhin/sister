...
while($datax 	=mysql_fetch_array($resultx))
{
	...
	while($j<$i)
	{
		...
		$pdf->Cell( 7.1	,0.4,'PERFORMANCE'				,'LRT',0,C,true); // 5.2
		$pdf->Cell( 7.1	,0.4,'PROJECTS/ASSIGNMENTS'	,'LRTB',0,C,true);// 5.2
		$pdf->Cell( 3.6	,0.4,'QUIZZES'		,'LRT',0,C,true);
		$pdf->Cell( 1	,0.4,'MID','LRT',0,C,true);
		
		$pdf->Cell( 1.6	,0.4,'QG','LRT',0,C,true); // QUARTER GRADE
		...
		$pdf->Cell( 1.6	,0.4,''		,'LRB',0,C,true); // QUARTER GRADE
