	...
	function R1D04HPGTK1()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_hdrkmnps_pgtk1.*
						...
						WHERE 		t_hdrkmnps_pgtk1.nis		='". mysql_escape_string($nis)."'	"; // menghasilkan nilai kehadiran per siswa
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			...
			{
				/*echo"
				<INPUT TYPE='button'	VALUE='Edit Attenance' 	onClick=window.location.href='guru.php?mode=R1D04HPGTK1&kdekls=$kdekls&pilihan=edit'>";*/
				echo"
				<INPUT TYPE='button'	VALUE='Edit Comment' 	onClick=window.location.href='guru.php?mode=R1D04HPGTK1&kdekls=$kdekls&pilihan=komentar'>";
				/*echo"
				<INPUT TYPE='button'	VALUE='Edit Promoted' 	onClick=window.location.href='guru.php?mode=R1D04HPGTK1&kdekls=$kdekls&pilihan=kenaikan'>";*/
			}	
			...
			/*if($pilihan=='edit')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04HPGTK1_Save'>
			...
			/*if($pilihan=='kenaikan')
			{
				echo"
				<INPUT TYPE='submit' 					VALUE='Simpan'>
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04HPGTK1_Save_Kenaikan'>
				<INPUT TYPE='hidden' NAME='kdekls'		VALUE=$kdekls>
				<INPUT TYPE='hidden' NAME='sms'			VALUE=$sms>
				<INPUT TYPE='hidden' NAME='nlitrm'		VALUE=$nlitrm>
				<INPUT TYPE='hidden' NAME='i'			VALUE=$i>";
	...
	/*function R1D04HPGTK1_Save()
	{
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_hdrkmnps.nis		='". mysql_escape_string($nis)."',
								...
								t_hdrkmnps.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_hdrkmnps.*
						FROM 		t_hdrkmnps
						WHERE 		t_hdrkmnps.nis	='". mysql_escape_string($nis)."'";
			$result =mysql_query($query);
		...
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04HPGTK1&kdekls=$kdekls&nis=$nis&pilihan=detil\">\n";
 	}	*/	
	...
	function R1D04HPGTK1_Save_Komentar()
	{
		...
		while($j<$i)
		{
			...
			$query	="	SELECT 		t_hdrkmnps_pgtk1.*
						FROM 		t_hdrkmnps_pgtk1
						WHERE 		t_hdrkmnps_pgtk1.nis	='". mysql_escape_string($nis)."'";
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
