...
while($data =mysql_fetch_array($result))
{
	if($data[kdeplj]!='')
	{
		...
		$query3 	="	SELECT 		t_prgrptps_sd.*
						...
						// menghasilka satu siswa per subjek (nilai akhir q1/q2/q3/q4)
...
while($y<$x)
{
		...
		if($kdeplj!='')
		{
			...
			$query2 ="	SELECT 		t_prgrptps_sd.*
						...
						WHERE		t_prgrptps_sd.nis='$nis'		AND 
									...
									t_prgrptps_sd.kdeplj='$kdeplj'"; //t_prgrptps_sd.thn_ajaran='". mysql_escape_string($thnajr)."' AND
									// menghasilka per siswa per subjek (nilai akhir q1/q2/q3/q4)
			$result2=mysql_query($query2) or die('Query gagal');
			$data2 =mysql_fetch_array($result2);
	...
	$pdf->Cell( 0.25	,0.4,"",0,0,L);
	$pdf->Cell(17.5	,0.6, "PRIMARY ".substr($kdekls,-2)." - SAINT JOHN'S SCHOOL",0,0,C);// 19
	$pdf->Ln();
	...
	$pdf->Cell( 16.3	,0.6,'ACADEMIC PERFORMANCE'		,'LRTB',0,C,true);
	
	$pdf->Ln();
	$pdf->SetFont('Arial','B',12);
	$pdf->Cell( 1.25	,0.6,'',0,0,L); 
	$pdf->Cell( 7.8	,1.8,'SUBJECT'		,'LRTB',0,C,true);
	$pdf->Cell( 7	,0.6,'CLASSROOM PERFORMANCE'		,'LRTB',0,C,true);
	$pdf->Cell( 1.5	,1.8,'ATT'		,'LRTB',0,C,true);//ATTITUDE
	
	$pdf->Ln(0.6);
	$pdf->Cell( 1.25	,0.6,'',0,0,L); 
	$pdf->Cell( 7.8	,1.8,''		,0,0,C,false);
	$pdf->Cell( 2.25	,0.6,'K'		,'LRTB',0,C,true);//KNOWLEDGE
	$pdf->Cell( 2.25	,0.6,'S'		,'LRTB',0,C,true);//SKILLS
	$pdf->Cell( 2.5		,0.6,'AVE'		,'LRTB',0,C,true);//AVERAGE
	
	$pdf->Ln(0.6);
	$pdf->Cell( 1.25	,0.6,'',0,0,L); 
	$pdf->Cell( 7.8	,1.8,''		,0,0,C,false);
	$pdf->Cell( 1.5	,0.6,'SC'		,'LRTB',0,C,true);//SCORE
	$pdf->Cell( 0.75,0.6,'LG'		,'LRTB',0,C,true);
	$pdf->Cell( 1.5	,0.6,'SC'		,'LRTB',0,C,true);//SCORE
	$pdf->Cell( 0.75,0.6,'LG'		,'LRTB',0,C,true);
	$pdf->Cell( 1.5	,0.6,'SC'		,'LRTB',0,C,true);//SCORE
	$pdf->Cell( 1	,0.6,'LG'		,'LRTB',0,C,true);
	$pdf->Ln();
	...
	while ($id<99)
	{
		...
		if($nmasbj!='')
		{
			...
			$pdf->SetFont('Arial','',12);
			$pdf->Cell( 1.5	,0.6,$q1K,'LRTB',0,C,true);
			$pdf->Cell( 0.75,0.6,$lgK,'LRTB',0,C,true);//K
			$pdf->Cell( 1.5	,0.6,$q1S,'LRTB',0,C,true);//S
			$pdf->Cell( 0.75,0.6,$lgS,'LRTB',0,C,true);
			$pdf->Cell( 1.5	,0.6,$ave1KS,'LRTB',0,C,true);//number_format(($cell[$i][4]+$cell[$i][5])/2,0,',','.');
			$pdf->Cell( 1	,0.6,$lgKS,'LRTB',0,C,true);
	...
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 1.25	,0.6,'',0,0,L);
	$pdf->Cell( 16.3,0.6,'EXTRA-CURRICULAR ACTIVITY PERFORMANCE','LRTB',0,C,true);//+1.5
	
	$pdf->Ln();
	$pdf->SetFont('Arial','',12);
	$pdf->Cell( 1.25	,0.6,'',0,0,L);
	$pdf->Cell( 6.8	,0.6,'ACTIVITY','LRTB',0,C,true);
	$pdf->Cell( 9.5	,0.6,'REMARKS','LRTB',0,C,true);
	...
	$qPMK	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='PMK' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND
							// extra kurikuler
	$rPMK=mysql_query($qPMK) or die('Query gagal40');
	$dPMK =mysql_fetch_array($rPMK);
	$q1PMK=$dPMK['ext'.$sms.$midtrm]; // q1 PMK	ktr
	...
	if($q1PMK!='')
	{
		...
		if($q1PMK=='A+')
			...
			$ktrWaj = "Sometimes not attending activities and     shows cooperation but is inconsistent.";
		else //if($q1PMK=='-')
			$ktrWaj = '-';
	}
	...
	$qKAR	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='KAR' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rKAR=mysql_query($qKAR) or die('Query gagal40');
	$dKAR =mysql_fetch_array($rKAR);
	$q1KAR=$dKAR['ext'.$sms.$midtrm]; // q1 KAR	ktr
	...
	$qRBT	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='RBT' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	...
	$qCLB	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='CLB' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	...
	$qCNM	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='CNM' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	...
	$qBDM	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='BDM' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rBDM=mysql_query($qBDM) or die('Query gagal40');
	$dBDM =mysql_fetch_array($rBDM);
	$q1BDM=$dBDM['ext'.$sms.$midtrm]; // q1 BDM	ktr
	//futsal
	$qFTS	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
							
							t_extcrrps.kdeplj='FTS' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	...
	$qBSK	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='BSK' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rBSK=mysql_query($qBSK) or die('Query gagal40');
	$dBSK =mysql_fetch_array($rBSK);
	$q1BSK=$dBSK['ext'.$sms.$midtrm]; // q1 BSK	ktr
	//theatre art
	$qTHE	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
							
							t_extcrrps.kdeplj='THE' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rTHE=mysql_query($qTHE) or die('Query gagal40');
	$dTHE =mysql_fetch_array($rTHE);
	$q1THE=$dTHE['ext'.$sms.$midtrm]; // q1 THE	ktr
	//home economics
	$qHCN	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
							
							t_extcrrps.kdeplj='HCN' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rHCN=mysql_query($qHCN) or die('Query gagal40');
	$dHCN =mysql_fetch_array($rHCN);
	$q1HCN=$dHCN['ext'.$sms.$midtrm]; // q1 HCN	ktr
	//karate
	$qKAR	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
	...
	$qCHO	="	SELECT 		t_extcrrps.*
				...
				WHERE		t_extcrrps.nis='$nis'	AND 
							...
							t_extcrrps.kdeplj='CHO' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	$rCHO=mysql_query($qCHO) or die('Query gagal40');
	$dCHO =mysql_fetch_array($rCHO);
	$q1CHO=$dCHO['ext'.$sms.$midtrm]; // q1 CHO	'ktr'
	//dancing
	$qMDR	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
							 
							t_extcrrps.kdeplj='MDR' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND
							// extra kurikuler
	$rMDR=mysql_query($qMDR) or die('Query gagal40');
	$dMDR =mysql_fetch_array($rMDR);
	$q1MDR=$dMDR['ext'.$sms.$midtrm]; // q1 MDR	'ktr'
	//creative art
	$qCART	="	SELECT 		t_extcrrps.*
				FROM 		t_extcrrps
				WHERE		t_extcrrps.nis='$nis'	AND 
							
							t_extcrrps.kdeplj='CART' "; //t_extcrrps.thn_ajaran='". mysql_escape_string($thnajr)."' AND 
							// extra kurikuler
	...
	if( $ktrWaj=='-' )
		$pdf->Cell( 9.5	,0.6,'','LRT',0,J,true);
	else
		$pdf->Cell( 9.5	,0.6,substr($ktrWaj,0,43),'LRT',0,J,true);//'Regularly attends and actively participates in '
	...
	if( $ktrWaj=='-' )
		$pdf->Cell( 9.5	,0.6,'-','LR',0,C,true);
	else
		$pdf->Cell( 9.5	,0.6,substr($ktrWaj,43,48),'LR',0,J,true);//'school activities and shows great enthusiasm in '
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,substr($ktrWaj,91,47),'LRB',0,J,true);//'performing school activities'
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'2','LRTB',0,C,true);
	$pdf->Cell( 6.2	,1.8,$nmaPil,'LRTB',0,C,true);
	$pdf->Cell( 9.5	,0.6,substr($ktrPil,0,43),'LRT',0,J);
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,substr($ktrPil,43,48),'LR',0,J);//B 63,63
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,substr($ktrPil,91,47),'LRB',0,J);
	...
	$qABS	="	SELECT 		t_hdrkmnps.*
				FROM 		t_hdrkmnps
				WHERE		t_hdrkmnps.nis='$nis'  "; //AND t_hdrkmnps.thn_ajaran='". mysql_escape_string($thnajr)."'
	...
	$pdf->Cell( 0.75,0.6,'   '.$q1SKT.'   ','T',0,C,true);
	$pdf->SetFont('Arial','',12);
	...
	$pdf->SetFont('Arial','U',12);
	$pdf->Cell( 0.75,0.6,'   '.$q1IZN.'   ',0,0,C,true);
	$pdf->SetFont('Arial','',12);
	...
	$pdf->SetFont('Arial','U',12);
	$pdf->Cell( 0.75,0.6,'   '.$q1ALP.'   ','B',0,C,true);
	$pdf->SetFont('Arial','',12);
