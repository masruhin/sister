...
//Keterangan	: Fungsi-fungsi INPUT PROGRESS REPORT			sd/ps
//----------------------------------------------------------------------------------------------------
if(!defined("sister"))
{
...
class R1D04Xclass
{
	...
	function R1D04X_Cari()
	{
		...
		/*$query	="	SELECT 		DISTINCT t_mstplj.kdeplj,t_mstplj.nmaplj,t_klmkls.kdetkt
								FROM 		t_mstpng,t_mstplj,t_mstkls,t_klmkls
								WHERE		t_mstpng.kdegru='$kdekry' AND
											t_mstpng.kdeplj=t_mstplj.kdeplj	AND
											t_mstpng.kdekls = t_mstkls.kdekls AND
				...
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					...
					while($data =mysql_fetch_array($result))
					{
						...
						<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
							<TD>$data[nmaplj]</TD>
							<TD><CENTER><a href='guru.php?mode=R1D04X&kdeplj=$data[kdeplj]&kdetkt=$data[kdetkt]'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>
						</TR>";
					}
				echo"	
				</TABLE>
	...
	function R1D04X()
	{
		...
		while($data = mysql_fetch_array($result)) // .. d
		{
			...
			//{
				...
				while($y<9)
				{
					$cell[$i][$y+9]=$data2['prj'."$sms"."$nlitrm"."$y"];
					$y++;
				}	
				...
				while($y<8)
				{
					$cell[$i][$y+17]=$data2['tes'."$sms"."$nlitrm"."$y"];
					$y++;
				}

				$cell[$i][$y+24]=$data2['midtes'."$sms"."$nlitrm"];
			...
			<DIV style='overflow:auto;;width:100%;height:340px;padding-right:-2px;'>
				...
				//{
					echo"<TR bgcolor='dedede'>
						...
						<TD WIDTH='25%' COLSPAN='8' HEIGHT='20'><CENTER>Knowledge Assessments (KA)	    	</CENTER></TD>
						<TD WIDTH='25%' COLSPAN='8'><CENTER>Skills Assessments (SA)	    	</CENTER></TD>
						<TD WIDTH='24%' COLSPAN='7'><CENTER>Spiritual & Social Behavior (7 CORE VALUES)	    	</CENTER></TD>";
						if ($pilihan=='detil'  AND $kdekls!='')
							echo"
							<TD WIDTH='4%' ROWSPAN='2'><CENTER><a href='guru.php?mode=R1D04X&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=midtes'><span style='color: #FF0000;'><b>mid</b></span></CENTER></TD>";
						else
							echo"
					...
					<TR bgcolor='dedede'>";
						if ($pilihan=='detil'  AND $kdekls!='')
						{
							...
							while($y<9)
							{
								echo"<TD WIDTH='4%' HEIGHT='20'><CENTER><a href='guru.php?mode=R1D04X&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=hw&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<9)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04X&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=prj&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<8)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04X&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=tes&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
						}
						else
	...
	function R1D04X_Save()
	{
		...
		while($j<$i)
		{
			...
			$query	="	SELECT 		t_prgrptps_sd.*
						FROM 		t_prgrptps_sd
						WHERE 		t_prgrptps_sd.nis	='". mysql_escape_string($nis)."'	AND
									t_prgrptps_sd.kdeplj	='". mysql_escape_string($kdeplj)."'"; // menghasilkan nis dan matpel nya
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
		...
		while($j<$i)
		{
			...
			$query2 ="	SELECT 		t_prgrptps_sd.*
						...
						WHERE		t_prgrptps_sd.nis	='$nis'		AND
									t_prgrptps_sd.kdeplj='$kdeplj'"; // menghasilkan nis dan matpel nya
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_prgrptps_sd."."$akh"."	='". mysql_escape_string($nliakh)."'"; 
			$query 	="	UPDATE 	t_prgrptps_sd ".$set.
					"	WHERE 	t_prgrptps_sd.nis	='". mysql_escape_string($nis)	."'	AND
								t_prgrptps_sd.kdeplj	='". mysql_escape_string($kdeplj)	."'";
