...
while($data =mysql_fetch_array($result))
{
	...
	$query2	="	SELECT 		distinct t_indbhvps.nis,t_indbhvps.idkdeplj,t_indbhvps.kdeplj
				FROM 		t_indbhvps
				WHERE		t_indbhvps.nis='$nis'	AND
							t_indbhvps.kdeplj!=''
				ORDER BY	t_indbhvps.nis,t_indbhvps.idkdeplj";
	...
	while($data2 = mysql_fetch_array($result2))
	{
		...
		$query5	="	SELECT 		t_prgrptps.*
					FROM 		t_prgrptps
					WHERE		t_prgrptps.nis='$nis'	AND
								t_prgrptps.kdeplj='$plj'";
		...
		while($y<6)
		{
			$cell[$i][$y+2]=$data5['hw'."$sms"."$midtrm"."$y"];
			$jml=$jml+$cell[$i][$y+2];
			if($cell[$i][$y+2]==0)
				$cell[$i][$y+2]='-';
					
			$cell[$i][$y+7]=$data5['prj'."$sms"."$midtrm"."$y"];
			$jml=$jml+$cell[$i][$y+7];
			if($cell[$i][$y+7]==0)
				$cell[$i][$y+7]='-';
					
			$cell[$i][$y+12]=$data5['tes'."$sms"."$midtrm"."$y"];
			$jml=$jml+$cell[$i][$y+12];
			if($cell[$i][$y+12]==0)
				$cell[$i][$y+12]='-';
		...
		if($data5[nis]!='' AND $jml>0)
		{
			$query4	="	SELECT 		t_setpsrpt.*
						FROM 		t_setpsrpt
						WHERE		t_setpsrpt.kdetkt='$kdetkt'	AND
									t_setpsrpt.kdeplj='$plj'";
			$result4=mysql_query($query4) or die('Query gagal');
			$data4 	=mysql_fetch_array($result4);
			$nmasbj	=str_replace("*","",$data4[nmasbj]);
			$cell2[$x][0] 	=str_replace("=","",$nmasbj);
			$x++;
		}	
	}
	...
	while($data3 = mysql_fetch_array($result3))
	{
		...
		{
			$cell3[$z][0]=ucwords(strtolower($data3['nmaplj'])).' : '.$data3['ext'."$sms"."$midtrm"];
