...
// -------------------------------------------------- Class --------------------------------------------------
class R1D04HPGTK4_AKRclass
{
	function R1D04HPGTK4_AKR()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_hdrkmnps_pgtk4_akr.*
						FROM 		t_hdrkmnps_pgtk4_akr
						WHERE 		t_hdrkmnps_pgtk4_akr.nis		='". mysql_escape_string($nis)."'	"; // menghasilkan nilai kehadiran per siswa
		...
		<FORM ACTION='guru.php' METHOD='get' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD COLSPAN='2'><B>INPUT NARASI (IV. PENGEMBANGAN KOGNITIF)</B></TD></TR>
				<TR></TR><TR></TR>
				...
				<TR><TD WIDTH='10%'>Class</TD>
					<TD WIDTH='90%'>:
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04HPGTK4_AKR'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			...
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit Comment' 	onClick=window.location.href='guru.php?mode=R1D04HPGTK4_AKR&kdekls=$kdekls&pilihan=komentar'>";
			...
			if($pilihan=='komentar')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04HPGTK4_AKR_Save_Komentar'>
	...
	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HPGTK4_AKR_Save()
	
	// -------------------------------------------------- Save --------------------------------------------------
	function R1D04HPGTK4_AKR_Save_Komentar()
	{
		$kdekls	=$_POST['kdekls'];
		$sms	=$_POST['sms'];
		$nlitrm	=$_POST['nlitrm'];
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_hdrkmnps_pgtk4_akr.nis		='". mysql_escape_string($nis)."',
								t_hdrkmnps_pgtk4_akr.kmn"."$sms"."$nlitrm"."	='". mysql_escape_string($kmn)."',
								t_hdrkmnps_pgtk4_akr.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_hdrkmnps_pgtk4_akr.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_hdrkmnps_pgtk4_akr.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_hdrkmnps_pgtk4_akr.*
						FROM 		t_hdrkmnps_pgtk4_akr
						WHERE 		t_hdrkmnps_pgtk4_akr.nis	='". mysql_escape_string($nis)."'";
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_hdrkmnps_pgtk4_akr ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan"));
			}
			else
			{
				$query 	="	UPDATE 	t_hdrkmnps_pgtk4_akr ".$set.
						"	WHERE 	t_hdrkmnps_pgtk4_akr.nis	='". mysql_escape_string($nis)	."'";
				$result =mysql_query ($query) or die (error("Data tidak berhasil di rubah"));		
			}	
			$j++;
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04HPGTK4_AKR&kdekls=$kdekls&nis=$nis&pilihan=detil\">\n";
 	}		

	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HPGTK4_AKR_Save_Kenaikan()
