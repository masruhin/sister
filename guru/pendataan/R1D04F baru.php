...
class R1D04Fclass
{
	...
	function R1D04F_Cari()
	{
		...
		<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
			<TR><TD><B>INPUT EXTRACURRICULAR</B></TD></TR>
			<TR></TR><TR></TR>
		</TABLE>
		...
		<FORM ACTION='guru.php?mode=R1D04F' METHOD='post'>
			<DIV style='overflow:auto;width:25%;height:380px;padding-right:-2px;'>		
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					<TR bgcolor='dedede'>
						<TD WIDTH='80%' HEIGHT='20'><CENTER>Personality Traits	</CENTER></TD>
	...
	function R1D04F()
	{
		...
		$query 	="	SELECT 		t_mstssw.*
					...
					ORDER BY	t_mstssw.kdekls,t_mstssw.nmassw";// menghasilkan semua murid per kelas
		$result =mysql_query($query);
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_extcrrps.*
						...
						WHERE 		t_extcrrps.nis		='". mysql_escape_string($nis)."'	AND
									t_extcrrps.kdeplj	='". mysql_escape_string($kdeplj)."'"; // menghasilkan nilai ektrakuriler per murid
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			...
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit' 	onClick=window.location.href='guru.php?mode=R1D04F&kdeplj=$kdeplj&kdekls=$kdekls&pilihan=edit'>";
				if($kdekls!='')
				{
					echo"
					<INPUT TYPE='button'	VALUE='Setting by Class' 	onClick=window.location.href='guru.php?mode=R1D04F&kdeplj=$kdeplj&kdekls=$kdekls&pilihan=seting'>";
				}	
			}	
