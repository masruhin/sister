...
class R1D04HSMPclass
{
	function R1D04HSMP()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_hdrkmnps_smp.*
						FROM 		t_hdrkmnps_smp
						WHERE 		t_hdrkmnps_smp.nis		='". mysql_escape_string($nis)."'	"; // menghasilkan nilai kehadiran per siswa
		...
		<FORM ACTION='guru.php' METHOD='get' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD COLSPAN='2'><B>Catatan Wali Kelas</B></TD></TR>
				<TR></TR><TR></TR>
				...
				<TR><TD WIDTH='10%'>Class</TD>
					<TD WIDTH='90%'>:
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04HSMP'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			<DIV style='overflow:auto;;width:100%;height:340px;padding-right:-2px;'>
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					<TR bgcolor='dedede'>
						...
						<TD WIDTH='45%'><CENTER>Catatan</CENTER></TD>
			...
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit Catatan' 	onClick=window.location.href='guru.php?mode=R1D04HSMP&kdekls=$kdekls&pilihan=komentar'>";
			...
			if($pilihan=='komentar')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04HSMP_Save_Komentar'>
	...
	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HSMP_Save()
	
	// -------------------------------------------------- Save --------------------------------------------------
	function R1D04HSMP_Save_Komentar()
	{
		$kdekls	=$_POST['kdekls'];
		$sms	=$_POST['sms'];
		$nlitrm	=$_POST['nlitrm'];
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_hdrkmnps_smp.nis		='". mysql_escape_string($nis)."',
								t_hdrkmnps_smp.kmn"."$sms"."$nlitrm"."	='". mysql_escape_string($kmn)."',
								t_hdrkmnps_smp.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_hdrkmnps_smp.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_hdrkmnps_smp.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_hdrkmnps_smp.*
						FROM 		t_hdrkmnps_smp
						WHERE 		t_hdrkmnps_smp.nis	='". mysql_escape_string($nis)."'";
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_hdrkmnps_smp ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan"));
			}
			else
			{
				$query 	="	UPDATE 	t_hdrkmnps_smp ".$set.
						"	WHERE 	t_hdrkmnps_smp.nis	='". mysql_escape_string($nis)	."'";
				$result =mysql_query ($query) or die (error("Data tidak berhasil di rubah"));		
			}	
			$j++;
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04HSMP&kdekls=$kdekls&nis=$nis&pilihan=detil\">\n";
 	}		

	// -------------------------------------------------- Save --------------------------------------------------
	//function R1D04HSMP_Save_Kenaikan()
