...
//Keterangan	: Fungsi-fungsi KURIKULUM plan
//----------------------------------------------------------------------------------------------------
if(!defined("sister"))
{
...
class R1D01H1class
{
	...
	function R1D01H1()
	{
		...
		if ($pilihan=='detil_general'  or $pilihan=='tambah_item' )
		{
			...
			$query 	="	SELECT 	g_gnrkrk.*
						FROM 	g_gnrkrk
						WHERE 	g_gnrkrk.kdekrk='". mysql_escape_string($kdemtrB)."'";
		...
		{	
			$id=$_GET['id'];
			$query1 ="	SELECT 		g_dtlkrk.*
						FROM 		g_dtlkrk
						WHERE 		g_dtlkrk.kdekrk='". mysql_escape_string($kdemtr)."'";
		...
		$query2 ="	SELECT 		g_dtlkrk.*,t_mstkry.*
					FROM  		g_dtlkrk,t_mstkry
					WHERE 		g_dtlkrk.kdekrk='". mysql_escape_string($kdemtr)."'AND
		...
		echo"
		<BODY onload='sf();'>
			<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01H1.js'></SCRIPT>
			
			<FORM ID='validasi'  ACTION='guru.php'METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					<TR><TD COLSPAN='2'><B>CURRICULUM PLAN SEMESTER 1</B></TD></TR>
					<TR></TR><TR></TR>
					...
					<INPUT TYPE='hidden' NAME='mode'  id='mode'	VALUE='R1D01H1_Save_Item'></TR>
				</TABLE>	
				...
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
								...
								<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>
									...
									<FORM  id='validasi'>
										<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>";
											...
											{   $id=$data['id'];
												...
												<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
													...
													if ($pilihan=='detil_general' or $pilihan=='tambah_item')
													{
                                                        ...
                                                        echo"
														<TD WIDTH='7%'><CENTER><a href='../files/semester1/$kdemtr/$nmamtr2.$type'target='_blank'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>";
                                                        if($kdegru!=$user)
                                                        {
                                                          echo"
											...
											<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01H1_hapus.js'></SCRIPT>
										</TABLE>
									</FORM>
								</TABLE>
							</DIV>	
					...
					<TR>
						<TD ALIGN='left'>
							<INPUT TYPE='button' VALUE='New' onClick=window.location.href='guru.php?mode=R1D01H1&pilihan=tambah_general'>";

							if ($pilihan=='detil_general')
							{
								echo"
								<INPUT TYPE='button' VALUE='Tambah Materi' onClick=window.location.href='guru.php?mode=R1D01H1&kdemtr=$kdemtr&pilihan=tambah_item'>
								<INPUT TYPE='button' VALUE='Hapus' onClick='confirmation()'>";
							}	
							if ($pilihan=='detil_general' or $pilihan=='tambah_item')
							{
								//echo"<INPUT TYPE='button' VALUE='Print' 	onClick=window.open('pendataan/R1D01H1_Cetak.php?kdemtr=$kdemtr')>";
							}
						echo"
						</TD>
					</TR>
	...
	function R1D01H1_Save()
	{
        ...
        $query1 ="	SELECT 		g_gnrkrk.*
					FROM 		g_gnrkrk
					WHERE
								g_gnrkrk.kdeklm	='". mysql_escape_string($kdeklm)."'	and
								g_gnrkrk.kdeplj	='". mysql_escape_string($kdeplj)."'";

		$result =mysql_query($query1);
		$data1 	=mysql_fetch_array($result);
		if($data1==0)
		{
			$set	="	SET     g_gnrkrk.kdekrk	='". mysql_escape_string($kdemtr)."',
								g_gnrkrk.kdeklm	='". mysql_escape_string($kdeklm)."',
								g_gnrkrk.kdeplj	='". mysql_escape_string($kdeplj)."'";

			$query 	="INSERT INTO g_gnrkrk ".$set;
			$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
        }
 	}
    // -------------------------------------------------- Save Detail --------------------------------------------------
	function R1D01H1_Save_Item()
	{
		require_once '../fungsi_umum/sysconfig.php';
        ...
        $set		="	SET     g_dtlkrk.kdekrk	='". mysql_escape_string($kdemtr)."',
                                g_dtlkrk.kdegru	='". mysql_escape_string($kdegru)."',
		   						g_dtlkrk.nmakrk	='". mysql_escape_string($nmamt)."',
                                g_dtlkrk.stt	='". mysql_escape_string($stt)."',
                                g_dtlkrk.tglinp	='". mysql_escape_string($tglinp)."',
                                g_dtlkrk.jaminp	='". mysql_escape_string($jaminp)."',
								g_dtlkrk.type	='". mysql_escape_string($type[1])."'";

  		$query 	="INSERT INTO g_dtlkrk ".$set;
		$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
		$kdegruB=$kdegru;

        $handle = is_dir("../files/semester1/$kdemtr");
        if($handle=='')
        {
			mkdir("../files/semester1/".$kdemtr, 0775);
        }
		...
		{
			$newfile= "../files/semester1/$kdemtr/$nmamt.$type[1]";
			if (file_exists($newfile))
				unlink($newfile);
			copy($pdf, "../files/semester1/$kdemtr/$nmamt.$type[1]");
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D01H1&kdemtr=$kdemtr&pilihan=tambah_item\">\n";
	}
