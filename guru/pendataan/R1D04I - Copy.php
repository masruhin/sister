				...
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					...
					while($data =mysql_fetch_array($result))
					{
						...
						<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
							<TD>$data[nmaplj]</TD>
							<TD><CENTER><a href='guru.php?mode=R1D04I&kdeplj=$data[kdeplj]&kdetkt=$data[kdetkt]'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>
						</TR>";
					}
				echo"	
				</TABLE>
			...
			else
			{
				$query2	="	SELECT 		t_prgrptps.*
						...
						WHERE 		t_prgrptps.nis		='". mysql_escape_string($nis)."'	AND
									t_prgrptps.kdeplj	='". mysql_escape_string($kdeplj)."'"; // menghasilkan nilai mata pelajaran per murid
				$result2 =mysql_query($query2);
				$data2 	 =mysql_fetch_array($result2);
				$y=1;
				while($y<6)
				...
				if($kdetkt=='PS')
				{
					echo"<TR bgcolor='dedede'>
						...
						if ($pilihan=='detil'  AND $kdekls!='')
							echo"
							<TD WIDTH='4%' ROWSPAN='2'><CENTER><a href='guru.php?mode=R1D04I&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=midtes'><span style='color: #FF0000;'><b>mid</b></span></CENTER></TD>";
						else
							echo"
					...
					<TR bgcolor='dedede'>";
						if ($pilihan=='detil'  AND $kdekls!='')
						{
							...
							while($y<9)
							{
								echo"<TD WIDTH='4%' HEIGHT='20'><CENTER><a href='guru.php?mode=R1D04I&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=hw&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<9)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04I&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=prj&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<8)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04I&kdeplj=$kdeplj&kdekls=$kdekls&kdetkt=$kdetkt&pilihan=edit&kolom1=tes&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
						}
						else
	...
	function R1D04I_Save()
	{
		...
		while($j<$i)
		{
			...
			$query	="	SELECT 		t_prgrptps.*
						FROM 		t_prgrptps
						WHERE 		t_prgrptps.nis	='". mysql_escape_string($nis)."'	AND
									t_prgrptps.kdeplj	='". mysql_escape_string($kdeplj)."'"; // menghasilkan nis dan matpel nya
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
		...
		while($j<$i)
		{
			...
			$query2 ="	SELECT 		t_prgrptps.*
						...
						WHERE		t_prgrptps.nis	='$nis'		AND
									t_prgrptps.kdeplj='$kdeplj'"; // menghasilkan nis dan matpel nya
