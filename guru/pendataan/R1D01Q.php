...
//Keterangan	: Fungsi-fungsi TQ upload
//----------------------------------------------------------------------------------------------------
if(!defined("sister"))
{
...
class R1D01Qclass
{
	...
	function R1D01Q()
	{
		...
		{
			$kdemtrB=$_GET['kdemtr'];
			$query 	="	SELECT 	g_gnrutq.*
						FROM 	g_gnrutq
						WHERE 	g_gnrutq.kdeutq='". mysql_escape_string($kdemtrB)."'";
		...
		{	
			$id=$_GET['id'];
			$query1 ="	SELECT 		g_dtlutq.*
						FROM 		g_dtlutq
						WHERE 		g_dtlutq.kdeutq='". mysql_escape_string($kdemtr)."'";
		...
		$query2 ="	SELECT 		g_dtlutq.*,t_mstkry.*
					FROM  		g_dtlutq,t_mstkry
					WHERE 		g_dtlutq.kdeutq='". mysql_escape_string($kdemtr)."'AND
                                g_dtlutq.kdegru=t_mstkry.kdekry
					ORDER BY g_dtlutq.nmautq";
		$result= mysql_query($query2) or die (mysql_error());

		echo"
		<BODY onload='sf();'>
			<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01Q.js'></SCRIPT>
			
			<FORM ID='validasi'  ACTION='guru.php'METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					<TR><TD COLSPAN='2'><B><!--TEST QUESTIONS-->SEMESTER PLAN</B></TD></TR>
					<TR></TR><TR></TR>
					...
					<INPUT TYPE='hidden' NAME='mode'  id='mode'	VALUE='R1D01Q_Save_Item'></TR>
				</TABLE>	
				...
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
								...
								<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>
									...
									<FORM  id='validasi'>
										<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>";
											...
											<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01Q_hapus.js'></SCRIPT>
										</TABLE>
									</FORM>
								</TABLE>
							</DIV>	
					...
					<TR>
						<TD ALIGN='left'>
							<INPUT TYPE='button' VALUE='New' onClick=window.location.href='guru.php?mode=R1D01Q&pilihan=tambah_general'>";

							if ($pilihan=='detil_general')
							{
								echo"
								<INPUT TYPE='button' VALUE='Tambah Materi' onClick=window.location.href='guru.php?mode=R1D01Q&kdemtr=$kdemtr&pilihan=tambah_item'>
								<INPUT TYPE='button' VALUE='Hapus' onClick='confirmation()'>";
							}	
							if ($pilihan=='detil_general' or $pilihan=='tambah_item')
							{
								//echo"<INPUT TYPE='button' VALUE='Print' 	onClick=window.open('pendataan/R1D01Q_Cetak.php?kdemtr=$kdemtr')>";
							}
						echo"
						</TD>
					</TR>
	...
	function R1D01Q_Save()
	{
        ...
        $query1 ="	SELECT 		g_gnrutq.*
					FROM 		g_gnrutq
					WHERE
								g_gnrutq.kdeklm	='". mysql_escape_string($kdeklm)."'	and
								g_gnrutq.kdeplj	='". mysql_escape_string($kdeplj)."'";

		$result =mysql_query($query1);
		$data1 	=mysql_fetch_array($result);
		if($data1==0)
		{
			$set	="	SET     g_gnrutq.kdeutq	='". mysql_escape_string($kdemtr)."',
								g_gnrutq.kdeklm	='". mysql_escape_string($kdeklm)."',
								g_gnrutq.kdeplj	='". mysql_escape_string($kdeplj)."'";

			$query 	="INSERT INTO g_gnrutq ".$set;
			$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
        }
 	}
    // -------------------------------------------------- Save Detail --------------------------------------------------
	function R1D01Q_Save_Item()
	{
		require_once '../fungsi_umum/sysconfig.php';
        ...
        $set		="	SET     g_dtlutq.kdeutq	='". mysql_escape_string($kdemtr)."',
                                g_dtlutq.kdegru	='". mysql_escape_string($kdegru)."',
		   						g_dtlutq.nmautq	='". mysql_escape_string($nmamt)."',
                                g_dtlutq.stt	='". mysql_escape_string($stt)."',
                                g_dtlutq.tglinp	='". mysql_escape_string($tglinp)."',
                                g_dtlutq.jaminp	='". mysql_escape_string($jaminp)."',
								g_dtlutq.type	='". mysql_escape_string($type[1])."'";

  		$query 	="INSERT INTO g_dtlutq ".$set;
		$result =mysql_query ($query) or die (error("Data tidak berhasil di Input"));
		$kdegruB=$kdegru;
		...
		{
			$newfile= "../files/materi/$kdemtr/$nmamt.$type[1]";
			if (file_exists($newfile))
				unlink($newfile);
			copy($pdf, "../files/materi/$kdemtr/$nmamt.$type[1]");
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D01Q&kdemtr=$kdemtr&pilihan=tambah_item\">\n";
	}
