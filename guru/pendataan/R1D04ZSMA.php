...
// -------------------------------------------------- Class --------------------------------------------------
class R1D04ZSMAclass
{
	// -------------------------------------------------- Layar Utama --------------------------------------------------
	function R1D04ZSMA_Cari()
	{
		...
		<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
			<TR><TD><B>INPUT GRADING SHEET (SHS) 12 <!--IPS--></B></TD></TR>
			<TR></TR><TR></TR>
		</TABLE>
			
		<FORM ACTION='guru.php?mode=R1D04ZSMA' METHOD='post'>
				...
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					...
					while($data =mysql_fetch_array($result))
					{
						...
						<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
							<TD>$data[nmaplj]</TD>
							<TD><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$data[kdeplj]'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>
						</TR>";
					}
				echo"	
				</TABLE>
	...
	function R1D04ZSMA()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_prgrptps_sma_p.*
						...
						WHERE 		t_prgrptps_sma_p.nis		='". mysql_escape_string($nis)."'	AND
									...
									t_prgrptps_sma_p.kdeplj	='". mysql_escape_string($kdeplj)."'";
			...
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				...
				<TR><TD WIDTH='15%'>Class</TD>
					<TD>:
						...
						while($data=mysql_fetch_array($result2))
						{
							...
							{
								if( $kdeplj=='CME' OR $kdeplj=='ART' OR $kdeplj=='ECN' OR $kdeplj=='GGRY' OR $kdeplj=='HIST' OR $kdeplj=='MTH' OR $kdeplj=='SCL' )
								{
				...
				<tr>
					<td>Tahun Pelajaran</td><td> : 
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04ZSMA'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			<DIV style='overflow:auto;;width:100%;height:340px;padding-right:-2px;'>
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					<TR bgcolor='dedede'>
						...
						<TD COLSPAN='12' HEIGHT='20'><CENTER>PERFORMANCE	    	</CENTER></TD>
						<TD COLSPAN='12'><CENTER>PROJECTS/ASSIGNMENTS	    	</CENTER></TD>
						<TD COLSPAN='5'><CENTER>QUIZZES	    	</CENTER></TD>";
						
						if ($pilihan=='detil'  AND $kdekls!='')
							echo"
							<TD WIDTH='4%' ROWSPAN='2'><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=edit&kolom1=midtes'><span style='color: #FF0000;'><b>mid</b></span></CENTER></TD>";
						else
							echo"
					...
					<TR bgcolor='dedede'>";
						if ($pilihan=='detil'  AND $kdekls!='')
						{
							...
							while($y<13)
							{
								echo"<TD WIDTH='4%' HEIGHT='20'><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=edit&kolom1=hw&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<13)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=edit&kolom1=prj&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
							$y=1;
							while($y<6)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=edit&kolom1=tes&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}
							...
							while($y<6)
							{
								echo"<TD WIDTH='4%'><CENTER><a href='guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=edit&kolom1=ag&kolom2=$y'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}
			...
			<BR>
			<INPUT TYPE='button' 	VALUE='Subject' 	onClick=window.location.href='guru.php?mode=R1D04ZSMA_Cari'>";
			if($pilihan=='detil' AND $kdekls!='')
			{
			...
			if($pilihan=='edit')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04ZSMA_Save'>
	...
	function R1D04ZSMA_Save()
	{
		...
		$query	="	SELECT 		t_mstbbt_sma_p.*
					FROM 		t_mstbbt_sma_p
					WHERE		t_mstbbt_sma_p.kdebbt='1HW'";
		...
		$query	="	SELECT 		t_mstbbt_sma_p.*
					FROM 		t_mstbbt_sma_p
					WHERE		t_mstbbt_sma_p.kdebbt='2PRJ'";
		$result	=mysql_query($query) or die('Query gagal1');
		$data 	=mysql_fetch_array($result);
		$bbtprj=$data[bbt];
		...
		$query	="	SELECT 		t_mstbbt_sma_p.*
					FROM 		t_mstbbt_sma_p
					WHERE		t_mstbbt_sma_p.kdebbt='3TES'";
		$result	=mysql_query($query) or die('Query gagal1');
		$data 	=mysql_fetch_array($result);
		$bbttes=$data[bbt];
		...
		$query	="	SELECT 		t_mstbbt_sma_p.*
					FROM 		t_mstbbt_sma_p
					WHERE		t_mstbbt_sma_p.kdebbt='4MID'";
		$result	=mysql_query($query) or die('Query gagal1');
		$data 	=mysql_fetch_array($result);
		$bbtmidtes=$data[bbt];
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_prgrptps_sma_p.nis		='". mysql_escape_string($nis)."',
								t_prgrptps_sma_p.kdeplj	='". mysql_escape_string($kdeplj)."',
								t_prgrptps_sma_p.thn_ajaran		='". mysql_escape_string($thn_ajr)."',
								t_prgrptps_sma_p."."$nliw"."	='". mysql_escape_string($nli)."',
								t_prgrptps_sma_p.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_prgrptps_sma_p.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_prgrptps_sma_p.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_prgrptps_sma_p.*
						FROM 		t_prgrptps_sma_p
						WHERE 		t_prgrptps_sma_p.nis	='". mysql_escape_string($nis)."'	AND
									t_prgrptps_sma_p.thn_ajaran	='". mysql_escape_string($thn_ajr)."'	AND	
									t_prgrptps_sma_p.kdeplj	='". mysql_escape_string($kdeplj)."'";
			...
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_prgrptps_sma_p ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan"));
			}
			else
			{
				$query 	="	UPDATE 	t_prgrptps_sma_p ".$set.
						"	WHERE 	t_prgrptps_sma_p.nis	='". mysql_escape_string($nis)	."'	AND
									t_prgrptps_sma_p.thn_ajaran	='". mysql_escape_string($thn_ajr)	."'	AND
									t_prgrptps_sma_p.kdeplj	='". mysql_escape_string($kdeplj)	."'";
		...
		while($j<$i)
		{
			...
			$query2 ="	SELECT 		t_prgrptps_sma_p.*
						...
						WHERE		t_prgrptps_sma_p.nis	='$nis'		AND
									...
									t_prgrptps_sma_p.kdeplj='$kdeplj'";
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_prgrptps_sma_p."."$avehw"."		='". mysql_escape_string(number_format($avghw))."',
								t_prgrptps_sma_p."."$aveprj"."	='". mysql_escape_string(number_format($avgprj))."',
								t_prgrptps_sma_p."."$avetes"."	='". mysql_escape_string(number_format($avgtes))."',
								t_prgrptps_sma_p."."$avemid"."	='". mysql_escape_string(number_format($midtes30))."',
								t_prgrptps_sma_p."."$akh"."	='". mysql_escape_string($nliakh)."',
								t_prgrptps_sma_p."."$aff"."	='". mysql_escape_string($ag_x)."' "; 
			$query 	="	UPDATE 	t_prgrptps_sma_p ".$set.
					"	WHERE 	t_prgrptps_sma_p.nis	='". mysql_escape_string($nis)	."'	AND
								t_prgrptps_sma_p.thn_ajaran	='". mysql_escape_string($thn_ajr)	."'	AND
								t_prgrptps_sma_p.kdeplj	='". mysql_escape_string($kdeplj)	."'";
		...
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04ZSMA&kdeplj=$kdeplj&kdekls=$kdekls&thn_ajr=$thn_ajr&pilihan=detil\">\n";
 	}		
}//akhir class
?>