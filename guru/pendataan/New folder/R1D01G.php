...
class R1D01Gclass
{
	...
	function R1D01G()
	{
		...
		if ($pilihan=='detil_general'  or $pilihan=='tambah_item' )
		{
			...
			$query 	="	SELECT 	g_gnrrpp.*
						FROM 	g_gnrrpp
						WHERE 	g_gnrrpp.kderpp='". mysql_escape_string($kderppB)."'";
		...
		{	
			...
			$query1 ="	SELECT 		g_dtlrpp.*
						FROM 		g_dtlrpp
						WHERE 		g_dtlrpp.kderpp='". mysql_escape_string($kderpp)."'";
		...
		$query2 ="	SELECT 		g_dtlrpp.*,t_mstkry.*
					...
					WHERE 		g_dtlrpp.kderpp='". mysql_escape_string($kderpp)."'AND
		...
		echo"
		<BODY onload='sf();'>
			<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01G.js'></SCRIPT>
			
			<FORM ID='validasi'  ACTION='guru.php'METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					<TR><TD COLSPAN='2'><B>LESSON PLAN</B></TD></TR>
					<TR></TR><TR></TR>
					<TR><TD WIDTH='10%'>Subject</TD>
						<TD WIDTH='90%'>:

							<INPUT TYPE='hidden' 	NAME='kderpp'  	id='kderpp'value='$kderpp'>
					...
					<INPUT TYPE='hidden' NAME='mode'  id='mode'	VALUE='R1D01G_Save_Item'></TR>
				</TABLE>	
				...
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
						...
						<TD WIDTH='100%' valign='top'>
							<div style='overflow:auto;width:100%;height:330px;padding-right:-2px;'>                
								<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>
									...
									<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' id='rpp' class='table02'>
									</TABLE>
									...
									<FORM  id='validasi'>
										<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='0' CELLSPACING='0' WIDTH='100%' class='table02'>";
											...
											{   $id=$data['id'];
												...
												<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
													...
													if ($pilihan=='detil_general' or $pilihan=='tambah_item')
													{
                                                        ...
                                                        echo"
														<TD WIDTH='7%'><CENTER><a href='../files/materi/$kderpp/$nmarpp2.$type'target='_blank'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>";
                                                        if($kdegru!=$user)
                                                        {
                                                          echo"
                                                        ...
                                                        {
                                                        echo"
														<TD WIDTH='7%'><CENTER><a href='#' id='$data[id]' typ='$type' nmamt='$nmarpp2' class='hapusrpp'><IMG src='../images/hapus_e.gif' BORDER='0'></a></CENTER></TD>";
                                                        }
													}
													else
													{
											...
											<SCRIPT TYPE='text/javascript' src='../guru/js/R1D01G_hapus.js'></SCRIPT>
										</TABLE>
									</FORM>
								</TABLE>
							</DIV>	
					...
					<TR>
						<TD ALIGN='left'>
							<INPUT TYPE='button' VALUE='New' onClick=window.location.href='guru.php?mode=R1D01G&pilihan=tambah_general'>";

							if ($pilihan=='detil_general')
							{
								echo"
								<INPUT TYPE='button' VALUE='Tambah Materi' onClick=window.location.href='guru.php?mode=R1D01G&kderpp=$kderpp&pilihan=tambah_item'>
								<INPUT TYPE='button' VALUE='Hapus' onClick='confirmation()'>";
							}	
							if ($pilihan=='detil_general' or $pilihan=='tambah_item')
							{
								//echo"<INPUT TYPE='button' VALUE='Print' 	onClick=window.open('pendataan/R1D01G_Cetak.php?kderpp=$kderpp')>";
							}
						echo"
						</TD>
					</TR>
	...
	function R1D01G_Save()
	{
		...
		if($data1==0)
		{
			$set	="	SET     g_gnrrpp.kderpp	='". mysql_escape_string($kderpp)."',
	...
	function R1D01G_Save_Item()
	{
		...
		$cari=array(",",":","'","\\","/","\"","*","?","<",">","|",".");
		$ganti=array(" ","-"," "," "," "," "," "," ","-"," ","-"," ");
		$rpp=str_replace($cari,$ganti,$nmarpp);
		$nmamt=trim($rpp);

        $set		="	SET     g_dtlrpp.kderpp	='". mysql_escape_string($kderpp)."',
        ...
        $handle = is_dir("../files/materi/$kderpp");
        if($handle=='')
        {
			mkdir("../files/materi/".$kderpp, 0775);
        }
		...
		{
			$newfile= "../files/materi/$kderpp/$nmamt.$type[1]";
			if (file_exists($newfile))
				unlink($newfile);
			copy($pdf, "../files/materi/$kderpp/$nmamt.$type[1]");
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D01G&kderpp=$kderpp&pilihan=tambah_item\">\n";
	}
