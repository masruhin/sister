...
// -------------------------------------------------- Class --------------------------------------------------
class R1D04XTK_AKRclass
{
	// -------------------------------------------------- Layar Utama --------------------------------------------------
	function R1D04XTK_AKR_Cari()
	{
		...
		$query	="	SELECT 		DISTINCT t_mstplj.kdeplj,t_mstplj.nmaplj
								...
								WHERE		t_mstpng.kdegru='$kdekry' AND
											...
											t_mstplj.str='' AND
											t_mstplj.kdeplj LIKE 'A%' ";
		$result	=mysql_query($query) or die (mysql_error()); // menghasilkan kode dan nama pelajaran

		echo"
		<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
			<TR><TD><B>INPUT LAPORAN PERKEMBANGAN ANAK</B></TD></TR>
			<TR></TR><TR></TR>
		</TABLE>
			
		<FORM ACTION='guru.php?mode=R1D04XTK_AKR' METHOD='post'>
				...
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%' class='table02'>
					...
					while($data =mysql_fetch_array($result))
					{
						...
						<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
							<TD>$data[nmaplj]</TD>
							<TD><CENTER><a href='guru.php?mode=R1D04XTK_AKR&kdeplj=$data[kdeplj]'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>
						</TR>";
					}
				echo"	
				</TABLE>
	...
	function R1D04XTK_AKR()
	{
		...
		while($data = mysql_fetch_array($result)) // .. d
		{
			...
			//if($kdetkt=='PS')
			//{
				$query2	="	SELECT 		t_learnrcd_tk_akr.*
						FROM 		t_learnrcd_tk_akr
						WHERE 		t_learnrcd_tk_akr.nis		='". mysql_escape_string($nis)."' AND
									t_learnrcd_tk_akr.kdeplj	='". mysql_escape_string($kdeplj)."' ";
				...
				$qry2 	="	SELECT 		t_lrcd_tk_akr.*
								FROM 		t_lrcd_tk_akr 
								WHERE		t_lrcd_tk_akr.kdekls	='". mysql_escape_string($kdekls)."' AND
											t_lrcd_tk_akr.kde LIKE '". mysql_escape_string($kdeplj)."%' ";
		...
		<FORM ACTION='guru.php' METHOD='get' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD COLSPAN='2'><B>INPUT LAPORAN PERKEMBANGAN ANAK<span style='color: #FF0000;'>( $nmaplj )</span></B></TD></TR>
				<TR></TR><TR></TR>
				...
				<TR><TD WIDTH='15%'>Class</TD>
					<TD>:
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04XTK_AKR'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
					...
					<TR bgcolor='dedede'>";
						if ($pilihan=='detil'  AND $kdekls!='')
						{
							...
							while($y<$yes)
							{
								//$sst = $cell[0][$y+1];
								$nmektr=$cell[$y][44];
								echo"<TD WIDTH='4%' HEIGHT='20'><CENTER><a href='guru.php?mode=R1D04XTK_AKR&kdeplj=$kdeplj&kdekls=$kdekls&pilihan=edit&kolom1=hw&kolom2=$y' title='$nmektr'><span style='color: #FF0000;'><b>$y</b></span></CENTER></TD>";
								$y++;
							}	
					...
					while($j<$i)
					{
						...
						<TR>
							...
							while($y<$yes)
							{
								...
								$y++;//ONKEYUP		='formatangka(this);' ONKEYPRESS	='return enter(this,event)'
							}	
			...
			<BR>
			<!--<INPUT TYPE='button' 	VALUE='Subject' 	onClick=window.location.href='guru.php?mode=R1D04XTK_AKR_Cari'>-->";
			if($pilihan=='detil' AND $kdekls!='')
			{
			...
			if($pilihan=='edit')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04XTK_AKR_Save'>
	...
	function R1D04XTK_AKR_Save()
	{
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_learnrcd_tk_akr.nis		='". mysql_escape_string($nis)."',
								t_learnrcd_tk_akr.kdekls	='". mysql_escape_string($kdekls)."',
								t_learnrcd_tk_akr.kdeplj	='". mysql_escape_string($kdeplj)."',
								t_learnrcd_tk_akr."."$nliw"."	='". mysql_escape_string($nli)."',
								t_learnrcd_tk_akr.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_learnrcd_tk_akr.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_learnrcd_tk_akr.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_learnrcd_tk_akr.*
						FROM 		t_learnrcd_tk_akr
						WHERE 		t_learnrcd_tk_akr.nis	='". mysql_escape_string($nis)."'	AND 
									t_learnrcd_tk_akr.kdekls	='". mysql_escape_string($kdekls)."'	AND
									t_learnrcd_tk_akr.kdeplj	='". mysql_escape_string($kdeplj)."'"; // menghasilkan nis dan matpel nya
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_learnrcd_tk_akr ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan")); // input nilai
			}
			else
			{
				$query 	="	UPDATE 	t_learnrcd_tk_akr ".$set.
						"	WHERE 	t_learnrcd_tk_akr.nis	='". mysql_escape_string($nis)	."'	AND 
									t_learnrcd_tk_akr.kdekls	='". mysql_escape_string($kdekls)."'	AND
									t_learnrcd_tk_akr.kdeplj	='". mysql_escape_string($kdeplj)	."'";
		...
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04XTK_AKR&kdeplj=$kdeplj&kdekls=$kdekls&pilihan=detil\">\n";
 	}		
}//akhir class
?>