...
while($y<$x)
{
	...
	while ($id<99)
	{
		...
		if($nmasbj!='')
		{
			...
			$pdf->Cell( 1.5	,0.6, $ave1KS,'LRTB',0,C,true);
	...
	$q1PMK=$dPMK['ktr'.$sms.$midtrm]; // q1 PMK
	...
	$q1BDM=$dBDM['ktr'.$sms.$midtrm]; // q1 BDM
	//basketball
	...
	$q1HCN=$dHCN['ktr'.$sms.$midtrm]; // q1 HCN
	//karate
	...
	$q1KAR=$dKAR['ktr'.$sms.$midtrm]; // q1 KAR
	//choir
	...
	$q1CHO=$dCHO['ktr'.$sms.$midtrm]; // q1 CHO
	...
	$pdf->Cell( 6.2	,1.8,'Praja Muda Karana (Pramuka)','LRTB',0,C,true);
	$pdf->Cell( 9.5	,0.6,'Regularly attends and actively participates in ','LRT',0,J,true);
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,'school activities and shows great enthusiasm in ','LR',0,J,true);
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,'performing school activities','LRB',0,J,true);
	...
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,substr($ktrPil,43,47),'LR',0,J);//B 63,63
	
	$pdf->Ln();
	$pdf->Cell( 1.25	,0.4,'',0,0,L);
	$pdf->Cell( 0.6	,1.8,'',0,0,C,false);
	$pdf->Cell( 6.2	,1.8,'',0,0,C,false);
	$pdf->Cell( 9.5	,0.6,substr($ktrPil,90,47),'LRB',0,J);
