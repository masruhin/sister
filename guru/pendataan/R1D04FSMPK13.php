...
// -------------------------------------------------- Class --------------------------------------------------
class R1D04FSMPK13class
{
	function R1D04FSMPK13()
	{
		...
		while($data = mysql_fetch_array($result))
		{
			...
			$query2	="	SELECT 		t_sciabhvps.*
						FROM 		t_sciabhvps
						WHERE 		t_sciabhvps.nis		='". mysql_escape_string($nis)."'	"; // menghasilkan nilai kehadiran per siswa
		...
		<FORM ACTION='guru.php' METHOD='get' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD COLSPAN='2'><B>INPUT Social Behaviour</B></TD></TR>
				<TR></TR><TR></TR>
				...
				<TR><TD WIDTH='10%'>Class</TD>
					<TD WIDTH='90%'>:
						...
						<INPUT TYPE='hidden'	NAME='mode'		VALUE='R1D04FSMPK13'>
		...
		<FORM ACTION='guru.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			...
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit' 	onClick=window.location.href='guru.php?mode=R1D04FSMPK13&kdekls=$kdekls&pilihan=edit'>";
				//
			...
			if($pilihan=='edit')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 		VALUE='R1D04FSMPK13_Save'>
	...
	function R1D04FSMPK13_Save()
	{
		...
		while($j<$i)
		{
			...
			$set	="	SET		t_sciabhvps.nis		='". mysql_escape_string($nis)."',
								t_sciabhvps.spr"."$sms"."$nlitrm"."	='". mysql_escape_string($spr)."',
								t_sciabhvps.des"."$sms"."$nlitrm"."	='". mysql_escape_string($des)."',
								
								t_sciabhvps.kdeusr	='". mysql_escape_string($kdeusr)."',
								t_sciabhvps.tglrbh	='". mysql_escape_string($tglrbh)."',
								t_sciabhvps.jamrbh	='". mysql_escape_string($jamrbh)."'"; 

			$query	="	SELECT 		t_sciabhvps.*
						FROM 		t_sciabhvps
						WHERE 		t_sciabhvps.nis	='". mysql_escape_string($nis)."'";
			$result =mysql_query($query);
			$data	=mysql_fetch_array($result);
			if($data[nis]=='')
			{
			  	$query 	="	INSERT INTO t_sciabhvps ".$set; 
				$result	=mysql_query($query) or die (error("Data tidak berhasil di simpan"));
			}
			else
			{
				$query 	="	UPDATE 	t_sciabhvps ".$set.
						"	WHERE 	t_sciabhvps.nis	='". mysql_escape_string($nis)	."'";
				$result =mysql_query ($query) or die (error("Data tidak berhasil di rubah"));		
			}	
			$j++;
		}
		echo"<meta http-equiv='refresh' content=\"0;url=guru.php?mode=R1D04FSMPK13&kdekls=$kdekls&nis=$nis&pilihan=detil\">\n";
 	}		
