...
$judul2=$nmasms.' MID TERM REPORT '.$midtrm;
...
while($data =mysql_fetch_array($result))
{
	if($data[kdeplj]!='')
	{
		...
		if($akh>0 OR $kdeplj=='MND' OR $kdeplj=='GRM' OR $kdeplj=='PHY' OR $kdeplj=='BLGY' OR $kdeplj=='CHM')
		{
...
while($y<$x)
{
	...
	while($data1 =mysql_fetch_array($result1))
	{
		...
		if($kdeplj!='')
		{
			...
			$cell[$i][4]=$data2['akh'."$sms"."$midtrm"];
			$nliakh		=$data2['akh'."$sms"."$midtrm"];
	...
	$pdf->SetFont('Arial','',8);
	$pdf->SetFillColor(255,255,255);
	$pdf->Cell( 19	,0.4,"Homeroom Teacher's Comments",'LRTB',0,C);
	$pdf->SetFont('Arial','',7);
	$pdf->Ln();
	$kmn 	=susun_kalimat($kmn, 160);
	$pdf->Cell( 19	,0.3,$kmn[0],'LRT',0,L);	
	$pdf->Ln();
	$pdf->Cell( 19	,0.3,$kmn[1],'LR',0,L);	
	$pdf->Ln();
	$pdf->Cell( 19	,0.3,$kmn[2],'LR',0,L);	
	$pdf->Ln();
	$pdf->Cell( 19	,0.3,$kmn[3],'LRB',0,L);	
	$pdf->SetFont('Arial','',8);
	$pdf->Ln();
	$pdf->Cell( 19	,0.4,"Parent's Comments",'LRTB',0,C);
	$pdf->Ln();
	$pdf->Cell( 19	,0.3,"",'LR',0,C);	
	$pdf->Ln();
	$pdf->Cell( 19	,0.3,"",'LRB',0,C);	
	$pdf->Ln();
