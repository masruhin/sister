...
{  //tampilkan soal di R1D01C//
   ...
   $(".kdeplj").change(function()
	...
	$(".more").click(function() 
	{
		...
		var id 			= element.attr("id");
		var kdercu 		= $("#kdercu").val();
		var kode 		= element.attr("kode");
		var dataString 	= 'id=' + id+'&kode=' + kode;
		...
		else
		{
			$.ajax(
			{
				...
				success: function()
				{
					$('#inputsoal').load("../guru/fungsi_khusus/R1D01C_inputsoalru.php?id="+id+"&kode="+kode);
				}
			});
		}
		return false;
