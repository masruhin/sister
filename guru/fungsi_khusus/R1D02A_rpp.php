...
$query	=" 	SELECT 		g_dtlrpp.*,g_gnrrpp.*,t_mstkry.*
			FROM 		g_dtlrpp,g_gnrrpp,t_mstkry
			WHERE 		g_gnrrpp.kdeplj='". mysql_escape_string($kdeplj)."'	AND
...
{   
	$nmamtr	=$data['nmarpp'];
	$nmamtr2	=substr($data['nmarpp'],0,30).'...';
    $kdemtr	=$data['kderpp'];
    $nmakry =$data['nmakry'];
    $tglinp =$data['tglinp'];
    $jaminp =$data['jaminp'];
