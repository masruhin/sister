...
{
	$('#kdemdl').blur(function()
	{
		//remove all the class add the messagebox classes and start fading
		$('#msgbox').removeClass().addClass('messagebox').text('Checking...').fadeIn('1000');
		//check the kdemdl exists or not from ajax
		$.post('../administrator/fungsi_khusus/A1D01_validasi_kdemdl.php',{ kdemdl:$(this).val() } ,function(data)
		{
			if(data=='no') //if kdemdl not avaiable
			{
				$('#msgbox').fadeTo(200,0.1,function() //start fading the messagebox
				{
					//add message and change the class of the box and start fading
					$(this).html('Kode sudah ada...').addClass('messageboxerror').fadeTo(900,1)
					document.f1.kdemdl.focus();
				});
			}
			else
			{
