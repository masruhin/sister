...
class P1D04class
{
	...
	function P1D04_Cari()
	{
		...
		$query2 ="	SELECT 		t_gnrpjm.*,t_mstssw.*
					FROM 		t_gnrpjm,t_mstssw
		...
		<FORM ACTION=perpustakaan.php METHOD='get' NAME='f1'>
			<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
				<TR><TD><B>PEMINJAMAN SISWA</B></TD></TR>
				<TR></TR><TR></TR>
				...
				<TR><TD>Tanggal Peminjaman</TD>
					<TD>:
						...
						<INPUT TYPE='hidden' 	NAME='mode'		VALUE='P1D04_Cari'>
						<INPUT TYPE='button' 					VALUE='Tampilkan Semua' onClick=window.location.href='perpustakaan.php?mode=P1D04_Cari'>
					</TD>
				</TR>
			</TABLE>
		</FORM>

		<FORM ACTION='perpustakaan.php?mode=P1D04' METHOD='post'>
				...
				<TABLE BORDER='1' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='0' WIDTH='100%'>
					...
					while($data =mysql_fetch_array($result))
					{
						...
						<TR onMouseOver=\"this.className='highlight'\" onMouseOut=\"this.className='normal'\">
							...
							<TD>$data[nmassw] ($data[nis] - $data[kdekls])</TD>";
							// otorisasi akses detil
							if (hakakses("P1D04D")==1)
							{
								echo"
								<TD><CENTER><a href='perpustakaan.php?mode=P1D04&nmrpjm=$data[nmrpjm]&pilihan=detilfull'><IMG src='../images/detil_e.gif' BORDER='0'></a></CENTER></TD>";
							}
							else
							{
								echo"
			...
			if (hakakses("P1D04T")==1)
			{
				echo"<INPUT TYPE='button' VALUE='Tambah PEMINJAMAN' onClick=window.location.href='perpustakaan.php?mode=P1D04&pilihan=tambah'>";
			}
		echo"
		</FORM>";
 	}
		...
		*/
		...
		<BODY onload='sf()'>
				...
				<TABLE BORDER='0' BORDERCOLOR='#000000' CELLPADDING='5' CELLSPACING='5' WIDTH='100%'>
					...
					<TR><TD>Kode Peminjam</TD>
						<TD>:   
							...
							<INPUT 	type='hidden' name='stt' 	id='stt' value='S' />
							<INPUT 	NAME		='kdeang'
									ID			='kdeang'
									TYPE		='text'
									SIZE		='10'
					...
					<TR><TD>Kode buku</TD>
						<TD>:
							...
							if ($pilihan!='detilfull')
							{
								if($isian=='enable')
								{
									echo"<INPUT TYPE='hidden' NAME='mode'		VALUE='P1D04_Save'>";
									echo"<INPUT TYPE='hidden' NAME='pilihan'	VALUE='tambah'>";
								}
								else
									echo"
									<INPUT TYPE='hidden' 	NAME='mode'		VALUE='P1D04_Save_Item'>
			...
			<FORM id='validasi'>
				...
				<BR>";
				// otorisasi akses tambah
				if (hakakses("P1D04T")==1 and ($pilihan=='detil' or $pilihan=='detilfull'))
				{
					echo"
					<INPUT TYPE='button' VALUE='Tambah PEMINJAMAN' onClick=window.location.href='perpustakaan.php?mode=P1D04&pilihan=tambah'>";
				}
				if ($pilihan=='detil' or $pilihan=='detilfull')
				{
					echo"
					<INPUT TYPE='button' VALUE='Cetak' 	onClick=window.open('pendataan/P1D04_Cetak.php?nmrpjm=$nmrpjm')>";
				}
				echo"
				<INPUT TYPE='button' 	VALUE='Cari' 	onClick=window.location.href='perpustakaan.php?mode=P1D04_Cari'>
