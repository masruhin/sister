...
$nmrpjm	=$_GET['nmrpjm'];
$query	="	SELECT 	t_gnrpjm.*,t_dtlpjm.*,t_mstkry.*,t_mstbku.*
			FROM 	t_gnrpjm,t_dtlpjm,t_mstkry,t_mstbku
			WHERE 	t_gnrpjm.nmrpjm='". mysql_escape_string($nmrpjm)."'	AND
					t_dtlpjm.nmrpjm=t_gnrpjm.nmrpjm						AND
					t_gnrpjm.kdeang=t_mstkry.kdekry						AND
...
while ($j<$i)
{
	...
	$pdf->Cell( 19,0,'',1); 	
	$pdf->Ln();
	$pdf->Cell(5	,0.5,'FORM : P1D06 (PERPUSTAKAAN)');
