...
$judul	='BOOK NOT RETURNED (BY PERSON)';
$tgl1	=$_POST['tgl1'];
$tgl2	=$_POST['tgl2'];
...
while($j<$i)
{
	...
	while($x<=60 and $j<$i)
	{
		...
		$z=0;
		while($x<=60 and $j<$i and $kdeang==$cll[$j][2])
		{
			if($z>0)	$pdf->Cell( 7.0	,0.4); // 7.6
			
			$jdl	=substr($cll[$j][5],0,46);
			$nmrpjm	=$cll[$j][0];
			$tglpjm	=$cll[$j][1];
			$jdwkmb	=$cll[$j][6];
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',6);
	$pdf->Cell( 5	,0.4,'FORM : P5L02_C01 (LIBRARY)');
	if ($j<$i)
	{
		$hlm++;
