...
while ($j<$i)
{
	...
	$pdf->Cell( 2.5	,0.4,'JUDUL'		,'LRTB',0,C);
	$pdf->Cell(13.2	,0.4,'DIPINJAM OLEH','LRTB',0,C);
	$pdf->Cell( 2.5	,0.4,'TANGGAL'		,'LRTB',0,C);
	$pdf->Ln();
	$pdf->SetFont('Arial','',7);
    $x=1;
	while ($x<=60 AND $j<$i)
	{
		...
		$pdf->Cell( 2.5	,0.4,$jdl				,0,0,C);
		$pdf->Cell(13.2	,0.4,$nma				,0,0,C);
		$pdf->Cell( 2.5	,0.4,$tgl				,0,0,L);
		$pdf->Ln();
		$Ttl=$Ttl+$nli;
		$No++;		
		$x++;
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',6);
	$pdf->Cell( 5	,0.4,'FORM : P5L04 (PERPUSTAKAAN)');
	if ($j<$i)
	{
		$hlm=$hlm+1;
		$pdf->Cell(19  	,0.4,"Bersambung ke hal : ".$hlm,0,0,R);
		$pdf->Ln();
	}
}
