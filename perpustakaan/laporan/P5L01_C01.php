...
//while($j<$jml)
//{
	$qry 	=mysql_query("	SELECT 		t_dtlpjm.*,t_gnrpjm.*,t_mstbku.*,t_mstssw.*,t_mstkls.*
							FROM 		t_dtlpjm,t_gnrpjm,t_mstbku,t_mstssw,t_mstkls
							WHERE 		(cast(concat(substring(t_gnrpjm.tglpjm,-4),'-',substring(t_gnrpjm.tglpjm,4,2),'-',substring(t_gnrpjm.tglpjm,1,2)) as date) between '$tgl3' and '$tgl4') and
										...
										t_gnrpjm.stt='S'															and
										((t_dtlpjm.tglkmb!='' and '$stt'='R') or (t_dtlpjm.tglkmb='' and '$stt'='N') or ('$stt'='')) and
										t_gnrpjm.kdeang = t_mstssw.nis 												and
										t_mstssw.kdekls = t_mstkls.kdekls 											and
										(t_mstkls.kdekls = '". mysql_escape_string($kdekls)."'	OR '$kdekls'='')
...
while($j<$i)
{
	...
	$pdf->Ln();
	$pdf->Cell(16.5	,0.4);	
	if($kdekls=='') {$kdekls='All';}
	$pdf->Cell( 2.5	,0.4,'Class : '.$kdekls,0,0,L);
	$pdf->Ln();
	...
	$pdf->Cell( 2.0	,0.4,'Number'	,'TB',0,L); // 1.7
	$pdf->Cell( 4.7	,0.4,'Member'	,'TB',0,L); // 5
	$pdf->Cell( 6.1	,0.4,'Book'		,'TB',0,L); // 5.8
	...
	$pdf->Ln();
	$pdf->SetFont('Arial','',6);
	$pdf->Cell( 5	,0.4,'FORM : P5L01_C01 (LIBRARY)');
	if ($j<$i)
	{
		$hlm++;
