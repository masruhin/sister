...
class L6U01SMPclass
{
	...
	function L6U01SMP()
	{
		...
		<FORM ID='validasi' ACTION='kurikulum.php' METHOD='post' NAME='f1' ENCTYPE='multipart/form-data'>
			<TABLE BORDER='0' CELLPADDING='0' CELLSPACING='5' WIDTH='100%'>
				<TR><TD><B>SET TAHUN AJARAN SMP</B></TD></TR>
				<TR></TR><TR></TR>";
					$j=0;
					while($j<4)
					{
			...
			// tombol edit
			if (hakakses('L6U01SMP')==1 and $pilihan=='detil')
			{
				echo"
				<INPUT TYPE='button'	VALUE='Edit' 	onClick=window.location.href='kurikulum.php?mode=L6U01SMP&set=$set&pilihan=edit'>";
			}	
			...
			if($pilihan=='edit')
			{
				...
				<INPUT TYPE='hidden' NAME='mode' 	VALUE='L6U01SMP_Save'>
	...
	function L6U01SMP_Save()
	{
		...
		while($j<4)
		{
			...
			$set	="	SET		t_setthn_smp.nli	='". mysql_escape_string($nli)."'";

			$query 	="	UPDATE 	t_setthn_smp ".$set. 
					 " 	WHERE 	t_setthn_smp.set	='". mysql_escape_string($setB)	."'";
			$result =mysql_query ($query) or die (error("Data tidak berhasil di Rubah"));
			$j++;
		}
		$pilihan='detil';
		
		echo"<meta http-equiv='refresh' content=\"0;url=kurikulum.php?mode=L6U01SMP&pilihan=$pilihan\">\n";
 	}		
}//akhir class
?>